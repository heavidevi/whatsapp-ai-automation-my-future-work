"""FacebookMessengerAdapter — Messenger Platform (Meta Send API), mock-first.

A FUTURE_PLACEHOLDER channel: live Messenger messaging requires a Facebook Page
access token, the `pages_messaging` permission, and app-review approval. Until
that's granted the adapter NEVER transmits — `send()` stays dry-run and explains
why.

The point of this file is to embed Messenger's hard messaging rule INSIDE the
adapter, isolated from the router:

**24-hour standard messaging window + message tags.** Messenger only allows
free-form (standard) replies within 24 hours of the user's last message. A
proactive send OUTSIDE that window MUST carry an approved message tag. If outside
the window and no tag is supplied, the send is BLOCKED here — it never reaches
the Send API.
...
# Messenger message text body cap (~2000 chars per send).
_MAX_TEXT_LEN = 2000


class FacebookMessengerAdapter(ChannelAdapter):
    """Facebook Messenger adapter (FUTURE_PLACEHOLDER, app-review gated, dry-run)."""

    def __init__(self) -> None:
        self.channel = Channel.FACEBOOK_MESSENGER
        self.support_level = support_level_for(Channel.FACEBOOK_MESSENGER)
    ...
    def capabilities(self) -> Capabilities:
        return Capabilities(
            ...
            outbound=True,
            supports_media=True,
            supports_buttons=True,  # button templates / quick replies
            supports_proactive=False,  # outside the 24h window needs a message tag
            max_text_len=_MAX_TEXT_LEN,
            notes=(
                "Facebook Messenger via Meta Send API (FUTURE_PLACEHOLDER, "
                "page access token + app-review required for pages_messaging). "
                "24-hour standard messaging window: proactive sends outside it "
                "require an approved message tag."
            ),
        )

    # ── inbound ──────────────────────────────────────────────────────────────
    def normalize_inbound(self, tenant_id: str, raw: dict) -> InboundMessage:
        """Normalize a Messenger webhook payload.
        ...
        return InboundMessage(
            ...
            sender_id=sender_id,
            thread_id=sender_id,  # Messenger threads by the user's PSID
            text=text,
            media=media,
            timestamp=timestamp,
    ...
    def format_outbound(self, msg: OutboundMessage) -> dict:
        """Shape into a Messenger Send API-shaped payload (recipient + message,
    ...
    def send(self, msg: OutboundMessage, config: ChannelConfig) -> SendResult:
        """Never transmits. Enforces the 24h standard-messaging window + message-
        tag rule, then dry-runs.

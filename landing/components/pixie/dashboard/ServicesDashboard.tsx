...
/**
 * ServicesDashboard — the user's Pixie "command center": every service Pixie can
 * run, each shown WITH the requirements it needs to start AND its live readiness
 * pulled from the backend channels/agents API (when that service runs).
 *
 * Themed to match the landing page (PixieMasterHero): #02070a background, a top
 * accent glow, white text, font-display headings, and accent-tinted glass cards.
 * Each card carries its own --accent/--soft (from the same PIXIE_SERVICES source
 * the product pages use) so a service looks identical here and on its page.
 * Readiness is real: backend down → static requirements view (no fake "ready").
 */
...
function ReadinessPill({
  ...
  // Service with no backend agent (website-builder / ai-influencer) → static.
  ...
  // unavailable (backend down) → fall back to the static "to start" count.
  return (
    <span className={`${base} border border-white/12 bg-white/[0.04] text-white/55`}>
      {reqCount} to start
    </span>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h3 className="mt-4 font-display text-xl font-extrabold tracking-tight text-[#F4FFF9]">
          {svc.serviceLabel}
        </h3>
...
export function ServicesDashboard({ tenant = 'demo' }: { tenant?: string }) {
  ...
  return (
    <div
      className="pixie-dashboard relative min-h-screen overflow-x-clip bg-[#02070a] text-white"
      style={{ ['--accent' as string]: '#8B7CF6', ['--soft' as string]: '#C4B5FD', ['--button-text' as string]: '#0a0612' }}
    >
      {/* Top accent glow — echoes the master hero's studio backdrop. */}
      <div
        aria-hidden
      ...
      {/* Top bar — same chrome as the product pages. */}
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-5 pb-24 pt-10 md:px-8">
        {/* Heading — mirrors the hero intro typography. */}
        <div className="max-w-2xl">
          <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-white/45">
            Your Pixie services
          </p>
          <h1 className="mt-3 font-display text-[clamp(2rem,4.2vw,3rem)] font-extrabold leading-[1.05] tracking-tight text-[#F4FFF9] text-balance">
            Pick a service. Pixie tells you exactly what it needs.
          </h1>
          <p className="mt-4 max-w-xl text-base leading-relaxed text-white/65">
            Every Pixie mode is ready to set up — each one lists the few details it needs from you,
            plus its live status once it&apos;s running. No long briefs, no heavy forms.
          </p>
        </div>
        ...
        >
          {services.map((svc, i) => (
            <ServiceCard
              ...
              index={i}
              loading={loading}
              readiness={data?.services?.[svc.slug]}
            />
          ))}
        </motion.div>

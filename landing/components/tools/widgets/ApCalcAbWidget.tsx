...
// AP Calculus AB exam structure (2024-2026):
//   MCQ: 45 questions (30 no-calc + 15 calc) → weighted to 50% of composite
//   FRQ: 6 questions × 9 pts = 54 raw → weighted to 50% of composite

const MAX_MCQ = 45;
const MAX_FRQ = 54;

const CUTOFFS: { score: 5 | 4 | 3 | 2 | 1; minComposite: number }[] = [
  { score: 5, minComposite: 67 },
  { score: 4, minComposite: 52 },
  { score: 3, minComposite: 37 },
  { score: 2, minComposite: 22 },
...
export function ApCalcAbWidget() {
  const [mcqRaw, setMcqRaw] = useState('30');
  const [frqRaw, setFrqRaw] = useState('36');
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          />
          <div className="mt-1 text-xs text-ink-400">30 no-calc + 15 calculator, 50% of composite</div>
        </div>
        <div>
          <label className="mb-1 block text-sm font-semibold text-ink-700">
          ...
          <input
            ...
            className="w-full rounded-xl border border-ink-200 bg-white px-3 py-3 text-base text-ink-900 outline-none transition focus:border-wa-green focus:ring-2 focus:ring-wa-green/20"
          />
          <div className="mt-1 text-xs text-ink-400">6 questions × 9 pts = 54 max</div>
        </div>
      </div>
      ...
      {result ? (
        <div className="rounded-2xl bg-gradient-to-br from-wa-bubble via-white to-wa-green/10 p-6 ring-1 ring-wa-green/20">
          ...
          {(() => {
            const cta = buildToolPrefill('ap-calc-ab-score-calculator', {

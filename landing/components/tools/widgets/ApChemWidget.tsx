...
//   FRQ: 3 long (10 pts each) + 4 short (4 pts each) = 46 raw → weighted to 50% of composite
// Composite ranges from 0 to 100. Cutoffs below are based on recent College Board curves.

const MAX_MCQ = 60;
const MAX_FRQ = 46;

// Approximate cutoffs (composite percentage of total).
// Real cutoffs vary year-to-year; these are within ±2 points of recent curves.
...
export function ApChemWidget() {
  const [mcqRaw, setMcqRaw] = useState('42');
  const [frqRaw, setFrqRaw] = useState('30');
  ...
  const result = useMemo(() => {
    ...
    // Each section contributes 50% to composite (scaled to 0-100).
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <input
            ...
            className="w-full rounded-xl border border-ink-200 bg-white px-3 py-3 text-base text-ink-900 outline-none transition focus:border-wa-green focus:ring-2 focus:ring-wa-green/20"
          />
          <div className="mt-1 text-xs text-ink-400">3 long × 10 + 4 short × 4 = 46 max</div>
        </div>
      </div>

...
// AP Psychology exam structure (2024-2026):
//   MCQ: 100 questions → weighted to 66.7% of composite
//   FRQ: 2 questions × 7 pts = 14 raw → weighted to 33.3% of composite

const MAX_MCQ = 100;
const MAX_FRQ = 14;
const MCQ_WEIGHT = 2 / 3;
const FRQ_WEIGHT = 1 / 3;

const CUTOFFS: { score: 5 | 4 | 3 | 2 | 1; minComposite: number }[] = [
  { score: 5, minComposite: 75 },
  { score: 4, minComposite: 60 },
  { score: 3, minComposite: 44 },
  { score: 2, minComposite: 28 },
...
export function ApPsychWidget() {
  const [mcqRaw, setMcqRaw] = useState('75');
  const [frqRaw, setFrqRaw] = useState('9');
  ...
  const result = useMemo(() => {
    ...
    // MCQ is 66.7%, FRQ is 33.3% — scale each to their contribution out of 100
    const mcqPct = (m / MAX_MCQ) * 100 * MCQ_WEIGHT;
    const frqPct = (f / MAX_FRQ) * 100 * FRQ_WEIGHT;
    ...
    // How many extra MCQ or FRQ points to hit next band
    const mcqToNext = pointsToNext ? Math.ceil((pointsToNext / (MCQ_WEIGHT * 100)) * MAX_MCQ) : null;
    const frqToNext = pointsToNext ? Math.ceil((pointsToNext / (FRQ_WEIGHT * 100)) * MAX_FRQ) : null;

    return { mcqPct, frqPct, composite, apScore, pointsToNext, nextScore, mcqToNext, frqToNext };
  }, [mcqRaw, frqRaw]);
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          />
          <div className="mt-1 text-xs text-ink-400">70 minutes, 66.7% of composite</div>
        </div>
        <div>
          <label className="mb-1 block text-sm font-semibold text-ink-700">
          ...
          <input
            ...
            className="w-full rounded-xl border border-ink-200 bg-white px-3 py-3 text-base text-ink-900 outline-none transition focus:border-wa-green focus:ring-2 focus:ring-wa-green/20"
          />
          <div className="mt-1 text-xs text-ink-400">2 questions × 7 pts = 14 max, 33.3% of composite</div>
        </div>
      </div>
      ...
      {result ? (
        <div className="rounded-2xl bg-gradient-to-br from-wa-bubble via-white to-wa-green/10 p-6 ring-1 ring-wa-green/20">
          ...
          <div className="mt-6 grid gap-4 border-t border-wa-green/20 pt-4 sm:grid-cols-2">
            <StatBlock label="MCQ contribution (66.7%)" value={`${result.mcqPct.toFixed(1)} / 66.7`} />
            <StatBlock label="FRQ contribution (33.3%)" value={`${result.frqPct.toFixed(1)} / 33.3`} />
          </div>

          {result.pointsToNext !== null && result.nextScore && result.mcqToNext !== null && result.frqToNext !== null && (
            <div className="mt-4 rounded-lg bg-white px-4 py-3 text-sm text-ink-500 shadow-soft">
              ...
              <strong>{result.mcqToNext}</strong> more MCQs or{' '}
              <strong>{result.frqToNext}</strong> more FRQ point{result.frqToNext === 1 ? '' : 's'}.
            </div>
          )}

          <p className="mt-4 text-xs text-ink-400">
            Estimate based on recent College Board curves (±1 score band). Real cutoffs adjust yearly.
          </p>

          {(() => {
            const cta = buildToolPrefill('ap-psych-score-calculator', {

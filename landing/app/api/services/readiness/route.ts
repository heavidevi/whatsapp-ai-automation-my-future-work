 ...
 * returns a compact per-service readiness summary the dashboard can badge with.
 *
 * Honest by design: if the backend is unreachable (it's a separate FastAPI
 * service, often not running in local frontend dev), every mappable service
 * comes back `state: "unavailable"` and `backendUp: false` — the dashboard then
 * falls back to the static requirements checklist rather than faking "ready".
 *
 * Only four frontend services map to a backend agent today; website-builder and
 * ai-influencer have no channels agent, so they're intentionally absent here and
...
interface ServiceReadiness {
  ...
  channelsReady: number; // requirements met
  channelsLive: number; // enabled + live + requirements met
  missing: string[];
  state: ServiceState;
}
  ...
  const results = await Promise.all(
    entries.map(async ([slug, agent]) => {
      try {
        ...
        // Backend down / timeout / network — degrade honestly to "unavailable".
        return [slug, { slug, agent, channelsTotal: 0, channelsReady: 0, channelsLive: 0, missing: [], state: 'unavailable' as ServiceState }];
      }
    }),
  );
